# This script contains the functions that are used to simulate
# the multivariate time series data under the factor model.

source('armodel.r') 

# This function generates the 6 stationary factor process.
...
# This function generates the loading matrix.
...
# This function generates the observed time series.
...
ts.GEN <- function(K,TL){
	...
	VAR <- diag(1,(4*K)) * 0.01
	for (t in 2:(TL+1)){
		TS[,t] <- loading.GEN(K) %*% factor.GEN(TL,1,1)[,(t-1)] + MASS::mvrnorm(n=1,mu=MU,Sigma=VAR)
}
	return(TS)
}
