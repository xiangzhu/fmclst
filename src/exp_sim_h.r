# This script compares four methods on the simulated data.
# Complete linkage is used for the hierarchical clustering.
...
# Use the true number of factors (6).
...
FMcls <- hclust(dist(A), method='complete')
# OR: the original time series
ORcls <- hclust(dist(data[,-1]), method='complete') 
# DW: the Haar wavelet coefficient matrix
DWcls <- hclust(TSclust::diss.DWT(data[,-1]), method='complete') 
# AP: the AR(\infty) operator coefficient matrix
ARcls <- hclust(dist(AR.PIC(data[,-1])), method='complete')	
