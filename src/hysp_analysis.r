# This script applies the factor model clustering method on 
# a hyperspectral imaging time series collected at ANL.  
...
# Check the residual plot of the factor model.
Idt <- diag(1,p)
Idt <- diag(1,p)
Resid <- t( (Idt-A%*%t(A)) %*% t(Y) ) # each row of Resid
colnames(Resid) <- Yname
mvtsplot::mvtsplot(Resid, norm='internal', margin=F, main="Residual plot of the factor model")

# Visualize the clustering results based on factor modelling.
rownames(A) <- WL
hc <- hclust(dist(A),method='ward')
clus5 <- cutree(hc,5)
edgepal <- c('darkorange','grey','seagreen','deeppink','navy')
Type <- c(rep(1,T1),rep(2,T2),rep(3,T3),rep(4,T4),rep(5,T5))
tippal <- c('darkmagenta','deepskyblue','limegreen','red','black')
#png('fan_0819.png',width=600,height=600, unit='px')
plot(ape::as.phylo(hc),type='fan',label.offset=0.15,cex=0.75,tip.color=tippal[Type],edge.color=edgepal[clus5],edge.width=1.5)
