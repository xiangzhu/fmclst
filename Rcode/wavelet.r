...
WFE <- function(series){
    ...
    max.level <- as.integer(ceiling(logb(length(series[1, ]), 
        base = 2)))
    true.level <- as.integer(floor(logb(length(series[1, ]), 
        base = 2)))
    ...
    for (i in 1:nrow(series)) {
        wavdecomp <- wavDWT(series[i, ], n.levels = max.level, 
            wavelet = "haar")
 ...
 wavdecomp <- wavDWT(series[1, ], n.levels = final_level, 
        wavelet = "haar")
    out.series <- wavdecomp$data[[final_level + 1]]
    for (i in 2:nrow(series)) {
        wavdecomp <- wavDWT(series[i, ], n.levels = final_level, 
            wavelet = "haar")
        out.series <- rbind(out.series, wavdecomp$data[[final_level + 
            1]])
    }
    out.series
}
